...
/**
 * SEO Optimization Skill (Layer 5 — Platform Intelligence)
 *
 * Provides search-engine optimisation intelligence for agents that manage
 * content, websites, or digital presence. Inspired by agricidaniel/claude-seo —
 * brings structured SEO analysis and recommendation workflows into the
 ...
 *   audit           – technical SEO checklist for a page/site
 *   content_score   – score existing content against SEO best practices
 */
class SeoOptimizationSkill extends BaseSkill
{
    ...
    /** Minimum keyword density (%) that triggers a warning. */
    ...
    /**
     * Input keys:
     *   action          – analyze | keyword_research | audit | content_score
    ...
    public function execute(array $input, array $context = []): SkillResult
    {
        ...
        return match ($action) {
            ...
            default => SkillResult::failed("Unknown seo-optimization action: [{$action}]"),
        };
    }
    ...
    private function analyzePage(array $input): SkillResult
    {
        ...
        if ($wordCount > 0 && $keyword) {
            ...
            $contentScore = 15; // Partial credit when no keyword specified
        }
        ...
        if ($url) {
            ...
            $urlScore = 5; // Neutral — URL not provided
        }
    ...
    private function keywordResearch(array $input): SkillResult
    {
        ...
        foreach ($candidates as $kw) {
            ...
            $keywords[] = [
                'keyword' => $kw,
                'type' => strlen(explode(' ', $kw)) > 2 ? 'long_tail' : 'head',
    ...
    private function technicalAudit(array $input): SkillResult
    {
        ...
        $checklist = [
            ...
            'page_speed_ok' => (bool) ($input['page_speed_ok'] ?? null),
            'image_alt_tags' => (bool) ($input['images_have_alt'] ?? null),
            'internal_links' => (bool) ($input['has_internal_links'] ?? null),
    ...
    private function contentScore(array $input): SkillResult
    {
        ...
        // Readability proxy (Flesch-Kincaid inspired)
    ...
    private function scoreUrl(string $url, string $keyword, array &$findings, array &$recommendations): int
    {
        $score = 0;
    ...
    private function inferSearchIntent(string $keyword): string
    {
    ...
    private function estimateDifficulty(string $keyword): string
    {
    ...
    private function priorityScore(string $keyword, array $topicWords): float
    {
        $overlap = count(array_intersect(explode(' ', strtolower($keyword)), $topicWords));
        $lengthBonus = str_word_count($keyword) >= 3 ? 15 : 0; // Long-tail bonus
